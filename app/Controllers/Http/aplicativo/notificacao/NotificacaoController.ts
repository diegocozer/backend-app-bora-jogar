...
export default class JogadorController {
  public async getNotificacao({ response, params }: HttpContextContract) {
    ...
    const notificacao = await Notificacoes.query()
      .where('codigo_jogador_notificacao', jogador?.codigo_jogador).andWhereIn('codigo_time_notificacao', timesList)
      .andWhere('visualizada_notificacao', false)
    const notificacaoAvisos = await Notificacoes.query()
      .whereNull('codigo_jogador_notificacao').andWhereIn('codigo_time_notificacao', timesList)
      .andWhere('visualizada_notificacao', false)
    return response.status(201).json({
      data: [...notificacao, ...notificacaoAvisos],
    });
  }
  public async getEventosJogos({ response, params }: HttpContextContract) {
    const { codigo_pessoa } = params
    const jogador = await Jogador.findBy('codigo_pessoa_jogador', codigo_pessoa)
    const usuario = await Usuario.findBy('codpes_usu', jogador?.codigo_pessoa_jogador)
    const times = await Time.query().select().where('pres_tim', String(usuario?.codigo_usu))
    const timesList = times.map(time => time.$attributes.codigo_tim)
    if (jogador === null) { return }
    const eventos = await EventosJogo.query()
      .whereIn('codigo_time_evento', timesList).preload('time').preload('jogo', jogo => jogo.preload('diasemana'))
